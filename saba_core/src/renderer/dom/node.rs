...
pub struct Node {
    ...
    window: Weak<RefCell<Window>>, // DOMツリーをもつウインドウ。弱い参照（Weakポインタ）として保持
    parent: Weak<RefCell<Node>>, // ノードの親ノード
    first_child: Option<Rc<RefCell<Node>>>, // ノードの一番最初の子ノード。強い参照（Rcポインタ）
    last_child: Option<Rc<RefCell<Node>>>, // ノードの最後の子ノード。強い参照（Rcポインタ）
    previous_sibling: Option<Rc<RefCell<Node>>>, // ノードの前の兄弟ノード
